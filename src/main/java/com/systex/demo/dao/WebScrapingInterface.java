...
public interface WebScrapingInterface extends SqlObject {

	default List<SourceUrl> findALl() {
		try (Handle handle = getHandle()) {
			handle.registerRowMapper(BeanMapper.factory(SourceUrl.class));
			String sql = "SELECT * FROM SOURCEURL WHERE ISACTIVE = '1'";
			Query query = handle.createQuery(sql);
			return query.mapTo(SourceUrl.class).list();
		}
	}
